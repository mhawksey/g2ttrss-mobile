...
//end of #login function
    ...
    $('#mark-these-read, #menu-mark-read').unbind('click').click(function () {
        ...
        request.done(function (response) {
			...
			headlines.done(function(response) {
				if (initiator == "menu-mark-read"){
					$('#mark-these-read').click();
				}
			});
        });
    });
...
function getHeadlines(since, optFeed_limit) {
    ...
    data.limit = optFeed_limit || pref_Feed_limit;
    data.show_excerpt = 1;
    data.show_content = 1;
        ...
        $('.entry-header-body').unbind('click').click(function () {
			...
			post_url = post_url.replace(/\?([^#]*)/, function(_, search) {
 			...
 			$.sharedCount(post_url, function(data){
 					...
 					//$(".share_widget.post-"+post_id+" span#li-count").text(data.LinkedIn);
 					//$(".share_widget.post-"+post_id+" span#del-count").text(data.Delicious);
 			});
        });
